//替换sprintf_s为sprintf； sprintf_s不是标准库函数，在gcc/g++编译器中不通过    --by 袁雨田 2019.8.22
#ifndef _SPLITSTR_H_
#define _SPLITSTR_H_
...
public:
    ...
    static void AddLeftZeroStr( int isrc, const char *ssrc, int len, char *sDes )
	{
			...
			if ( ssrc == NULL )				sprintf( ibuf, "%d", isrc );
			else if( strlen(ssrc) == 0 )    sprintf( ibuf, "%d", isrc );
			else                            sprintf( ibuf, "%s", ssrc );
